...
type ScreenMode   = 'jar' | 'life' | 'daily' | 'balance' | 'history';
...
function InlineDatePicker({ value, onChange }: { value: string; onChange: (d: string) => void }) {
  ...
  function pickDay(day: number) {
    const d = `${viewYear}-${`${viewMonth + 1}`.padStart(2, '0')}-${`${day}`.padStart(2, '0')}`;
    onChange(d);
    setOpen(false);
  }
  ...
  return (
    <View>
      <Pressable onPress={() => setOpen(o => !o)} style={dp.trigger}>
        <Text style={dp.triggerIcon}>📅</Text>
        <Text style={dp.triggerText}>{label}</Text>
        <Text style={dp.triggerCaret}>{open ? '▲' : '▼'}</Text>
      </Pressable>
      {open && (
        <View style={dp.picker}>
          <View style={dp.navRow}>
            <Pressable onPress={prevMonth} style={dp.navBtn}><Text style={dp.navBtnText}>‹</Text></Pressable>
            <Text style={dp.navLabel}>{monthLabel}</Text>
            <Pressable onPress={nextMonth} style={dp.navBtn}><Text style={dp.navBtnText}>›</Text></Pressable>
          </View>
          <View style={dp.dayHeader}>
            {['S','M','T','W','T','F','S'].map((d, i) => (
              <Text key={i} style={dp.dayHeaderText}>{d}</Text>
            ))}
          </View>
          {weeks.map((week, wi) => (
            <View key={wi} style={dp.weekRow}>
              {week.map((day, di) => {
                if (!day) return <View key={di} style={dp.dayEmpty} />;
                const dateStr = `${viewYear}-${`${viewMonth + 1}`.padStart(2,'0')}-${`${day}`.padStart(2,'0')}`;
                const isSelected = dateStr === value;
                const isToday = dateStr === today;
                return (
                  <Pressable key={di} onPress={() => pickDay(day)}
                    style={[dp.day, isSelected && dp.daySelected, isToday && !isSelected && dp.dayToday]}>
...
const dp = StyleSheet.create({
  trigger:       { flexDirection: 'row', alignItems: 'center', backgroundColor: '#f2ebff', borderRadius: 18, paddingHorizontal: 16, paddingVertical: 14, gap: 10 },
  triggerIcon:   { fontSize: 18 },
  triggerText:   { flex: 1, color: '#241042', fontWeight: '700', fontSize: 15 },
  triggerCaret:  { color: '#9d8ab5', fontSize: 13 },
  picker:        { backgroundColor: '#fff', borderRadius: 20, borderWidth: 2, borderColor: '#e0d4ff', padding: 14, gap: 10, marginTop: 4 },
  navRow:        { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  navBtn:        { width: 36, height: 36, borderRadius: 999, backgroundColor: '#f2ebff', justifyContent: 'center', alignItems: 'center' },
  navBtnText:    { color: '#7c7cff', fontSize: 20, fontWeight: '900' },
  navLabel:      { color: '#241042', fontWeight: '800', fontSize: 15 },
  dayHeader:     { flexDirection: 'row', justifyContent: 'space-between' },
  dayHeaderText: { flex: 1, textAlign: 'center', color: '#9d8ab5', fontSize: 11, fontWeight: '700' },
  weekRow:       { flexDirection: 'row', gap: 2, marginTop: 2 },
  day:           { flex: 1, aspectRatio: 1, borderRadius: 10, justifyContent: 'center', alignItems: 'center', backgroundColor: '#f8f2ff' },
  dayEmpty:      { flex: 1, aspectRatio: 1 },
  daySelected:   { backgroundColor: '#7c7cff' },
  dayToday:      { borderWidth: 2, borderColor: '#7c7cff' },
  dayText:       { color: '#241042', fontWeight: '700', fontSize: 13 },
...
export default function App() {
  ...
  const funAvail    = Math.max(month.funBudget, 0);
  ...
  function addFunExpense() {
    ...
    const createdAt = new Date(funDate + 'T12:00:00').toISOString();
    updateMonth({ ...month, funExpenses: [{ id: `${Date.now()}`, amount: amt, category: funCat, note: funNote.trim(), createdAt }, ...month.funExpenses] });
  ...
  function addRegExpense() {
    ...
    if (Number.isNaN(amt) || amt <= 0) { Alert.alert('Enter an amount'); return; }
    const createdAt = new Date(regDate + 'T12:00:00').toISOString();
    updateMonth({ ...month, regExpenses: [{ id: `${Date.now()}`, amount: amt, category: regCat, note: regNote.trim(), createdAt }, ...month.regExpenses] });
  ...
  return (
    <SafeAreaView style={s.safeArea}>
      ...
      <ScrollView contentContainerStyle={s.content} showsVerticalScrollIndicator={false}>
        ...
        {screen === 'jar' ? (
          <>
            ...
            <View style={s.card}>
              ...
              <Pressable onPress={toggleSavings} style={[s.savingsToggle, month.savingsEnabled && s.savingsToggleActive]}>
                ...
                <View style={{ flex: 1, gap: 2 }}>
                  <Text style={s.subheading}>Track savings this month</Text>
                  <Text style={s.subtitle}>Log how much you're putting away, separate from your fun and regular budgets.</Text>
                </View>
              </Pressable>
            ...
            <View style={s.card}>
              ...
              <InlineDatePicker value={funDate} onChange={setFunDate} />
        ...
        {screen === 'life' ? (
          <>
            ...
            <View style={s.card}>
              ...
              <InlineDatePicker value={regDate} onChange={setRegDate} />
        ...
        {/* ═══════════════════════ BALANCE TAB ═══════════════════════ */}
        {screen === 'balance' ? (() => {
          // Merge all transactions into one chronological list
          type TxType = 'income' | 'fun' | 'reg' | 'savings';
          type Tx = { id: string; date: string; amount: number; type: TxType; label: string; note: string };

          const txs: Tx[] = [
            ...month.income.map(e => ({
              id: e.id, date: e.createdAt, amount: e.amount,
              type: 'income' as TxType, label: e.source, note: e.note,
            })),
            ...month.funExpenses.map(e => ({
              id: e.id, date: e.createdAt, amount: -e.amount,
              type: 'fun' as TxType,
              label: funCats.find(c => c.key === e.category)?.label ?? 'Fun',
              note: e.note,
            })),
            ...month.regExpenses.map(e => ({
              id: e.id, date: e.createdAt, amount: -e.amount,
              type: 'reg' as TxType,
              label: regCats.find(c => c.key === e.category)?.label ?? 'Regular',
              note: e.note,
            })),
            ...(month.savingsEnabled && month.savingsSetAside > 0 ? [{
              id: 'savings', date: selKey + '-15T00:00:00.000Z', amount: -month.savingsSetAside,
              type: 'savings' as TxType, label: 'Savings', note: 'Set aside this month',
            }] : []),
          ...
          const totalIn  = month.income.reduce((s, e) => s + e.amount, 0);
          const totalOut = funSpent + regSpent + (month.savingsEnabled ? month.savingsSetAside : 0);
          ...
          const txIcon: Record<TxType, string> = {
            income: '↑', fun: '↓', reg: '↓', savings: '→',
          };

          return (
            <>
              {/* Big balance card */}
              <View style={[s.card, { backgroundColor: netBalance >= 0 ? '#e8f5e9' : '#fce4ec' }]}>
                <Text style={s.title}>Account balance</Text>
                <Text style={s.subtitle}>{getPeriodLabel(selKey)}</Text>
                <Text style={[s.balanceBig, { color: netBalance >= 0 ? '#2c7a3b' : '#d14a76' }]}>
                  {netBalance >= 0 ? '+' : ''}{fmt(netBalance)}
                </Text>
                <Text style={s.subtitle}>
                  {netBalance >= 0
                    ? 'You are in the green this month.'
                    : 'Your expenses exceed your logged income this month.'}
                </Text>
              </View>

              {/* Breakdown */}
              <View style={s.card}>
                <Text style={s.title}>Breakdown</Text>
                <View style={[s.summaryRow, { backgroundColor: '#e8f5e9' }]}>
                  <Text style={s.summaryLabel}>↑ Total income</Text>
                  <Text style={[s.summaryValue, { color: '#2c7a3b' }]}>{fmt(totalIn)}</Text>
                </View>
                <View style={[s.summaryRow, { backgroundColor: '#fff0f3' }]}>
                  <Text style={s.summaryLabel}>↓ Regular expenses</Text>
                  <Text style={[s.summaryValue, { color: '#d14a76' }]}>−{fmt(regSpent)}</Text>
                </View>
                <View style={[s.summaryRow, { backgroundColor: '#fff5f0' }]}>
                  <Text style={s.summaryLabel}>↓ Fun expenses</Text>
                  <Text style={[s.summaryValue, { color: '#d14a76' }]}>−{fmt(funSpent)}</Text>
                </View>
                {month.savingsEnabled && month.savingsSetAside > 0 && (
                  <View style={[s.summaryRow, { backgroundColor: '#fffde7' }]}>
                    <Text style={s.summaryLabel}>→ Savings set aside</Text>
                    <Text style={[s.summaryValue, { color: '#7a5c00' }]}>−{fmt(month.savingsSetAside)}</Text>
                  </View>
                )}
                <View style={[s.summaryRow, { backgroundColor: netBalance >= 0 ? '#e8f5e9' : '#fce4ec' }]}>
                  <Text style={[s.summaryLabel, { fontWeight: '900' }]}>= Balance</Text>
                  <Text style={[s.summaryValue, { color: netBalance >= 0 ? '#2c7a3b' : '#d14a76', fontSize: 22 }]}>
                    {netBalance >= 0 ? '+' : ''}{fmt(netBalance)}
                  </Text>
                </View>
              </View>
              ...
              <View style={s.card}>
                <Text style={s.title}>Transaction log</Text>
                <Text style={s.subtitle}>Every income and expense in order, with your running balance after each one.</Text>
                {txsWithBalance.length === 0 && (
                  <Text style={s.subtitle}>Nothing logged yet. Add income and expenses to see your balance here.</Text>
                )}
                {txsWithBalance.map((tx, i) => (
                  <View key={tx.id + i} style={s.txRow}>
                    <View style={[s.txIcon, { backgroundColor: txColor[tx.type] }]}>
                      <Text style={s.txIconText}>{txIcon[tx.type]}</Text>
                    </View>
                    ...
                    <View style={s.txRight}>
                      <Text style={[s.txAmount, { color: tx.amount >= 0 ? '#2c7a3b' : '#d14a76' }]}>
                        {tx.amount >= 0 ? '+' : ''}{fmt(tx.amount)}
                      </Text>
                      <Text style={[s.txBalance, { color: tx.balance >= 0 ? '#2c7a3b' : '#d14a76' }]}>
...
const s = StyleSheet.create({
  ...
  // Balance screen
  balanceBig:   { fontSize: 52, fontWeight: '900', color: '#241042', letterSpacing: -1 },
  txRow:        { flexDirection: 'row', alignItems: 'center', gap: 12, paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: '#f1eaff' },
  txIcon:       { width: 40, height: 40, borderRadius: 999, justifyContent: 'center', alignItems: 'center' },
  txIconText:   { fontSize: 18, fontWeight: '900', color: '#241042' },
  txMiddle:     { flex: 1, gap: 2 },
  txLabel:      { color: '#241042', fontWeight: '800', fontSize: 15 },
  txMeta:       { color: '#9d8ab5', fontSize: 13 },
  txRight:      { alignItems: 'flex-end', gap: 2 },
  txAmount:     { fontWeight: '800', fontSize: 15 },
  txBalance:    { fontWeight: '700', fontSize: 12 },
  // Calendar
